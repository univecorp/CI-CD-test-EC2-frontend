...
import "./EditSkillAssessment.css";
import config from "../../../config/apiConfig";
import { MdRemoveCircle } from "react-icons/md";
import { AiFillPlusCircle } from "react-icons/ai";
function EditSkillAssessment({ role }) {
  ...
  // Fetch the existing skill assessment data for editing.
  useEffect(() => {
    axios
      .get(`${config.apiUrl}/skill-assessment/${id}`)
      .then((response) => {
        const skillAssessmentData = response.data;

        // Populate the form fields with the fetched data
        setFormData({
          title: skillAssessmentData.data.title,
          image: skillAssessmentData.data.image,
          category: skillAssessmentData.data.category,
          label: skillAssessmentData.data.label,
          questions: skillAssessmentData.data.questions,
          SkillAssessmentTopic: skillAssessmentData.data.SkillAssessmentTopic,
          SkillAssessmentForWhichOne:
            skillAssessmentData.data.SkillAssessmentForWhichOne,
        });
      })
      .catch((error) => {
  ...
  const handleSubmit = (event) => {
    ...
    axios
      .patch(
        `${config.apiUrl}/skill-assessment/update-question/${id}`,
        formDataToSend
      )
      .then((response) => {
        ...
        toast.success("Data is Edited successfully");
      })
      .catch((error) => {
  ...
  return (
    <>
      <Header />
      <h2 className="header-skill">Edit Skill Assessment Form</h2>
      <div className="main-div-skill container">
        <form onSubmit={handleSubmit}>
          ...
          <button
            type="button"
            onClick={addQuestion}
            className=" add-question-btn-skill"
          >
            Add Question
          </button>
...
export default EditSkillAssessment;
